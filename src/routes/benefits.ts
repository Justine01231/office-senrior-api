...
// Delete benefit application
  ...
  // ========================================
  // BENEFITS SPECIALIST DASHBOARD ENDPOINTS
  // ========================================

  // Get assigned seniors for Benefits Specialist
  .get('/my-seniors', async ({ user }) => {
    if (!user) {
      throw new Error('Authentication required');
    }

    console.log(`👥 [MY-SENIORS] Benefits Specialist ${user.userId} requesting assigned seniors`);

    // Verify user is staff with benefits department
    if (user.role !== 'staff') {
      return {
        success: false,
    ...
    // Get assigned seniors with their benefit statistics
    const assignedSeniors = await db
      .select({
        ...
        pendingApplicationsCount: sql<number>`COUNT(CASE WHEN ${benefitApplications.status} = 'pending' THEN 1 END)`.as('pendingApplicationsCount'),
        // Count total applications
        totalApplicationsCount: sql<number>`COUNT(${benefitApplications.id})`.as('totalApplicationsCount'),
      })
      .from(staffAssignments)
      .innerJoin(users, eq(staffAssignments.seniorId, users.id))
      .leftJoin(benefitApplications, eq(benefitApplications.seniorId, users.id))
      .where(
        and(
      ...
      .groupBy(
        ...
        users.address,
        users.dateOfBirth,
        staffAssignments.assignedAt,
        staffAssignments.id
      );

    console.log(`✅ [MY-SENIORS] Found ${assignedSeniors.length} assigned seniors for Benefits Specialist`);

    return {
      success: true,
      data: assignedSeniors.map(senior => ({
        id: senior.seniorId,
        ...
        statistics: {
          pendingApplications: senior.pendingApplicationsCount || 0,
          totalApplications: senior.totalApplicationsCount || 0
        }
      })),
      count: assignedSeniors.length
    };
  })

// Get recent benefits activity
  .get('/recent-activity', async ({ query, user, headers, set }) => {
    // Fallback: derive user directly from Authorization header if middleware
    // didn't populate it for some reason.
    const effectiveUser = user || getUserFromHeaders(headers);
    if (!effectiveUser) {
      console.log('🚫 [RECENT-ACTIVITY] Authentication required');
      set.status = 401;
      return {
        success: false,
        error: 'Authentication required',
    ...
    console.log(`📋 [RECENT-ACTIVITY] Getting recent benefits activity (limit: ${limitNum})`);

    // Get recent status changes from history
    const recentStatusChanges = await db.select({
      ...
      newStatus: applicationStatusHistory.newStatus,
      reason: applicationStatusHistory.reason,
      staffName: sql<string>`staff_users.first_name || ' ' || staff_users.last_name`,
      createdAt: applicationStatusHistory.updatedAt
    })
    .from(applicationStatusHistory)
    .innerJoin(benefitApplications, eq(applicationStatusHistory.applicationId, benefitApplications.id))
    .innerJoin(users, eq(benefitApplications.seniorId, users.id))
    .leftJoin(
      sql`users as staff_users`,
      sql`application_status_history.updated_by = staff_users.id`
    )
    .orderBy(desc(applicationStatusHistory.updatedAt))
    .limit(limitNum);
    ...
    const recentDocuments = await db.select({
      id: documents.id,
      type: sql<string>`'document_upload'`,
    ...
    .from(documents)
    .innerJoin(benefitApplications, eq(documents.applicationId, benefitApplications.id))
    .innerJoin(users, eq(benefitApplications.seniorId, users.id))
    .leftJoin(
      sql`users as staff_users`,
      sql`documents.uploaded_by = staff_users.id`
    )
    .where(sql`${documents.uploadedAt} IS NOT NULL`)
    .orderBy(desc(documents.uploadedAt))
    .limit(limitNum);

    // Get recent new applications
    const recentApplications = await db.select({
      id: benefitApplications.id,
      type: sql<string>`'new_application'`,
      applicationId: benefitApplications.id,
      applicationType: benefitApplications.applicationType,
      seniorName: sql<string>`${users.firstName} || ' ' || ${users.lastName}`,
      applicationStatus: benefitApplications.status,
      priority: benefitApplications.priority,
      createdAt: benefitApplications.createdAt
    })
    .from(benefitApplications)
    ...
    const allActivities = [
      ...recentStatusChanges.map(item => ({
        id: `status_${item.id}`,
        type: 'status_change',
        title: `Status Updated: ${item.previousStatus} → ${item.newStatus}`,
        description: item.reason || 'Status changed',
        seniorName: item.seniorName,
        staffName: item.staffName || 'System',
        applicationType: item.applicationType,
        applicationId: item.applicationId,
        createdAt: item.createdAt,
        icon: '🔄',
        color: item.newStatus === 'approved' ? 'success' : item.newStatus === 'rejected' ? 'error' : 'warning'
      })),
      ...recentDocuments.map(item => ({
        id: `doc_${item.id}`,
        type: 'document_upload',
        title: `Document Uploaded: ${item.documentName}`,
        description: `${item.documentType} - ${item.documentStatus}`,
        seniorName: item.seniorName,
        staffName: item.staffName || 'System',
        applicationType: item.applicationType,
        applicationId: item.applicationId,
      ...
      ...recentApplications.map(item => ({
        ...
        title: `New Application: ${item.applicationType}`,
        description: `Priority: ${item.priority} - Status: ${item.applicationStatus}`,
        seniorName: item.seniorName,
        applicationType: item.applicationType,
        applicationId: item.applicationId,
        ...
        createdAt: item.createdAt,
        icon: '📝',
        color: item.priority === 'urgent' || item.priority === 'high' ? 'error' : 'primary'
      }))
    ]
    .sort((a, b) => {
    ...
    console.log(`✅ [RECENT-ACTIVITY] Found ${allActivities.length} recent activities`);
...
// Get consolidated dashboard statistics
  .get('/dashboard/stats', async ({ user, headers, set }) => {
    const effectiveUser = user || getUserFromHeaders(headers);
    if (!effectiveUser) {
    ...
    console.log(`📊 [DASHBOARD-STATS] Getting consolidated statistics for user ${effectiveUser.userId}`);
    ...
    const applicationsByType = await db.select({
      applicationType: benefitApplications.applicationType,
      count: count()
    })
    .from(benefitApplications)
    ...
    const applicationsByPriority = await db.select({
      priority: benefitApplications.priority,
      count: count()
    })
    .from(benefitApplications)
    .groupBy(benefitApplications.priority);

    // Calculate counts
    const total = totalApplications[0]?.count || 0;
    const pending = applicationsByStatus.find(s => s.status === 'pending')?.count || 0;
    const approved = applicationsByStatus.find(s => s.status === 'approved')?.count || 0;
    const rejected = applicationsByStatus.find(s => s.status === 'rejected')?.count || 0;
    const underReview = applicationsByStatus.find(s => s.status === 'under_review')?.count || 0;

    // If user is Benefits Specialist, get their specific stats
    let assignedSeniorsCount = 0;
    let myPendingApplications = 0;
    
    if (effectiveUser.role === 'staff') {
      const myApplications = await db.select({ count: count() })
        .from(benefitApplications)
        .where(eq(benefitApplications.assignedTo, effectiveUser.userId));
      
      const myPending = await db.select({ count: count() })
        .from(benefitApplications)
        .where(
          and(
            eq(benefitApplications.assignedTo, effectiveUser.userId),
            eq(benefitApplications.status, 'pending')
          )
        );

      const assignedSeniors = await db.select({ count: count() })
        .from(staffAssignments)
        .where(
          and(
            eq(staffAssignments.staffId, effectiveUser.userId),
            eq(staffAssignments.isActive, true)
          )
        );

      assignedSeniorsCount = assignedSeniors[0]?.count || 0;
      myPendingApplications = myPending[0]?.count || 0;
    }

    console.log(`✅ [DASHBOARD-STATS] Statistics calculated: Total=${total}, Pending=${pending}, Approved=${approved}`);
    ...
    return {
      ...
      data: {
        ...
        myStats: effectiveUser.role === 'staff' ? {
          assignedSeniors: assignedSeniorsCount,
          pendingApplications: myPendingApplications
        } : null
      }
    };
  });
